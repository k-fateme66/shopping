...
const WishList = () => {
    ...
    return (
        <>
            {
                wish.map((item) => (
                    <div className="flex justify-between border-t border-slate-200 py-5" key={item.id}>
                        <Link to={`/product/${item.name}?id=${item.id}`} className="flex items-center md:basis-1/3">
