...
const WishReducer = (state, action) => {
    ...
    switch (action.type) {
        case 'TOGGLE_TO_WISH':
            ...
            const updateIndexWishList = updateWishList.findIndex((item) => item.id === action.payLoad.id);
            if (updateIndexWishList < 0) {
                ...
                const filterWishList = updateWishList.filter((item) => item.id !== action.payLoad.id);
